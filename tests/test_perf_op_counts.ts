...
import type { ScenarioConfig } from "../src/types/simulation.js";
 ...
 */
function getTestScenario(passengerCount: number): ScenarioConfig {
...
test("op-count budget: N=1000, 1 tick, spatial-hash queries <= 2500", () => {
  ...
  // Assertions.
  console.log(
    `Queries: ${counters.queries}, MaxCandidates: ${counters.maxCandidates}, Rebuilds: ${counters.rebuilds}`,
  );
...
test("heap allocation budget: N=1000, 1 tick, delta < 1.5x baseline", () => {
  ...
  console.log(
    `Heap delta: ${(heapDelta / 1_000_000).toFixed(2)} MB (baseline: ${(HEAP_DELTA_BASELINE / 1_000_000).toFixed(2)} MB)`,
  );
...
test("determinism check: two runs with same seed produce identical tick", () => {
  ...
  for (let i = 0; i < next1.passengers.length; i++) {
    ...
    // noUncheckedIndexedAccess types array access as T | undefined; assert both
    // passengers exist so the determinism comparisons below narrow cleanly.
    assert.ok(p1 !== undefined && p2 !== undefined, `Passenger ${i} missing`);
