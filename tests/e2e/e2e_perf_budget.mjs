...
async function main() {
	...
	if (isCI) {
		...
		console.log(`  Target: mean tick time < ${MAX_TICK_TIME_MS} ms (>= ${TICKS_PER_SEC_TARGET} ticks/sec)`);
		...
		if (meanTickTime > MAX_TICK_TIME_MS) {
			...
			console.error(`Expected >= ${TICKS_PER_SEC_TARGET} ticks/sec; got ${(1000 / meanTickTime).toFixed(1)}/sec`);
			process.exit(1);
		}

		console.log(`PASSED: mean tick time ${meanTickTime.toFixed(3)} ms < ${MAX_TICK_TIME_MS} ms`);
		console.log(`Achieved ${(1000 / meanTickTime).toFixed(1)} ticks/sec (target >= ${TICKS_PER_SEC_TARGET})`);
		process.exit(0);
	}
}
