...
async function main() {
	...
	try {
		...
		const imageLoaded = await page.locator(".ship-schematic").evaluate(
		...
		requireValue(imageLoaded, "Ship schematic image did not load.");

		const overlayStats = await page.locator(".passenger-overlay").evaluate(
			function inspectOverlay(svg) {
				const circles = Array.from(svg.querySelectorAll("circle"));
				const healthStates = new Set(circles.map(function getHealth(circle) {
		...
		requireValue(
			overlayStats.circleCount > 0,
			"Passenger overlay has no passenger dots.",
		);
		requireValue(
		...
		const sciencePressed = await page.getByRole("button", { name: "Science" })
			.getAttribute("aria-pressed");
		requireValue(
			sciencePressed === "true",
			"Science mode button was not selected.",
		);
