...
test("resolveOverlaps separates two coincident agents", () => {
  ...
  const dist = Math.sqrt(dx * dx + dy * dy);
  assert.ok(dist > 0, `Coincident agents should be separated. Got dist=${dist}`);
...
test("resolveOverlaps: three agents in a line, all overlapping", () => {
  ...
  assert.strictEqual(
    exactOverlapCount(result),
    0,
    "No exact-center coincidence after 3-agent resolve",
  );
});
...
test("G10.5d: run 100 ticks on feasible-density fixture, never exact-center coincidence", () => {
  ...
  for (let tick = 0; tick < TICKS; tick++) {
    ...
    assert.strictEqual(overlaps, 0, `Exact-center coincidence at tick ${tick}: ${overlaps} pairs`);
  }
});
