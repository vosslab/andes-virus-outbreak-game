...
export function advanceSimulationTick(
  ...
  const events = [...movementResult.events, ...progressionResult.events, ...exposureResult.events];
  const nextState = {
    tick: nextTick,
...
function generateAgentParamsWithState(
  ...
  const speedResult = normalRandom(randomState, distribution.speed.mean, distribution.speed.stddev);
...
function exposePassengers(
  ...
  for (const passenger of passengers) {
    ...
    const exposure = scenario.sepir_rates
      ...
      : computeExposureLegacy(passenger, passengers, contamination, scenario, currentRandomState);
...
function computeExposureSepir(
  ...
  const infectiousNeighbors = queryNeighborsWithinDistance(
    ...
    return (
      neighborPassenger &&
      (neighborPassenger.health === "pre_symptomatic" || neighborPassenger.health === "symptomatic")
    );
  });
  ...
  const mechanism = infectiousNeighbors.length > 0 ? "near_infectious_passenger" : "what_if_fomite";
...
function computeFomiteExposureProbability(
  ...
  const probability =
    contaminationLevel * scenario.fomite.surfaceExposureChance * Math.max(0, cleaningMultiplier);
  return probability;
}
